...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() {
		...
		Map<String, Integer> locationEarthquakeCount = new HashMap<String, Integer>();
		locationEarthquakeCount.put("Ocean", 0);

		// Iterate && Count
		for (Marker country : countryMarkers) {
			for (Marker earthquake : quakeMarkers) {

				// check the type of the earthquake, and update the map
				if (earthquake instanceof LandQuakeMarker) {

					// checking if this earthquake took place in the
					// corresponding country
					String countryName = ((LandQuakeMarker) earthquake).getCountry();
					if (country.getProperty("name").equals(countryName)) {
						// if the map contains the country, update the count
						if (locationEarthquakeCount.containsKey(countryName)) {
							locationEarthquakeCount.put(countryName, locationEarthquakeCount.get(countryName) + 1);
						} else {
							// if not,set the country as key, and the count as 1
							locationEarthquakeCount.put(countryName, 1);
						}
					}
				}
			}
		...
		for (Marker earthquake : quakeMarkers) {
			if (earthquake instanceof OceanQuakeMarker) {
				// update the Ocean counts
				locationEarthquakeCount.put("Ocean", locationEarthquakeCount.get("Ocean") + 1);
			}
		}

		System.out.println(locationEarthquakeCount);
