 ...
 * team
 * 
 * @author Mustafa Elbehery Date: July 17, 2015
 */
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {
		...
		// creating a marker for each PointFeature : By Mustafa
	...
	private SimplePointMarker createMarker(PointFeature feature) {
		...
		SimplePointMarker simplePointMarker = new SimplePointMarker(feature.getLocation(), feature.getProperties());

		// setting the properties of the marker based on its properties
		if (Float.parseFloat(feature.getProperty("magnitude").toString()) < THRESHOLD_LIGHT) {
			simplePointMarker.setRadius(4);
			simplePointMarker.setColor(color(0, 0, 255));
		} else if (Float.parseFloat(feature.getProperty("magnitude").toString()) >= THRESHOLD_LIGHT
				&& Float.parseFloat(feature.getProperty("magnitude").toString()) < THRESHOLD_MODERATE) {
			simplePointMarker.setRadius(8);
			simplePointMarker.setColor(color(255, 255, 0));
		} else if (Float.parseFloat(feature.getProperty("magnitude").toString()) >= THRESHOLD_MODERATE) {
	...
	private void addKey() {
		...
		rect(25, 50, 150, 250);
		fill(0,0,0);
		text("Eearthquake Key", 55, 80);
		// Red Key
		fill(255,0,0);
		ellipse(50,110,17,14);
		fill(0,0,0);
		text("5.0+ Magnitude", 65, 115);
		// Yellow Key
		fill(255,255,0);
		ellipse(50,170,17,14);
		fill(0,0,0);
		text("4.0+ Magnitude", 65, 175);
		// Blue Key
		fill(0,0,255);
		ellipse(50,230,17,14);
		fill(0,0,0);
